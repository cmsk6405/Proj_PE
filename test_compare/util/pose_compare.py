...
class PoseCompare:
    ...
    def inference(self, img) -> Tuple[Dict, Dict]:
        ...
        for result in results.boxes.data:
            ...
            if int(cls) == 0:  # 사람 클래스일 경우
                ...
                for i in range(33):
                    # temp = []
                    # temp.append(mp_result.pose_landmarks.landmark[i].x)
                    # temp.append(mp_result.pose_landmarks.landmark[i].y)
                    # temp.append(mp_result.pose_landmarks.landmark[i].visibility)
    ...
    def load_img(self, frame, dest: str):
        ...
        if type(frame) == str:
            ...
            # print(f"경로 배열 전환시 {img.shape}")

        else:
            # If array then convert to PIL
            print(f"캠 타입 확인하려고 {type(frame)}")
            # img = Image.fromarray(frame)
            img = frame

        print(f"프레임 타입 - {type(frame)}")
        output, angles = self.inference(img)

        # Apply inference 
        # output - boxes, labels, scores, keypoints, keypoints_scores 값들
        # output 중 사용하는것 boxex keypoints,  못찾으면 yolo의 bbox값 전달
        # angles - 아래의 결과
        # angles = {}
        # for k, v in kpts_angle.items():
        ...
        if dest == "trgt":
            ...
            self.img_trgt = F.to_pil_image(img)
        else:
            self.output_ref = output
            self.angle_ref = angles
            self.tensor_ref = img
            print(f"결과 확인용1 {type(img)}")

            self.img_ref = F.to_pil_image(img)
